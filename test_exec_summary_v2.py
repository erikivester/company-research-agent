#!/usr/bin/env python3
"""
Test script for executive summary generation with PDF output.
"""
...
def create_test_state(research_data: dict) -> ResearchState:
    ...
    state = {
        ...
        # Create briefings from the raw data
...
def _create_company_briefing(data: dict) -> str:
    ...
    parts = []
    for url, content in list(company_info.items())[:3]:
...
def _create_news_briefing(data: dict) -> str:
    ...
    parts = []
    for url, content in list(news.items())[:3]:
...
def _create_flw_briefing(data: dict) -> str:
    ...
    parts = []
    for url, content in list(sustainability.items())[:3]:
...
def _create_engagement_briefing(data: dict) -> str:
    ...
    parts = []
    for url, content in list(partnerships.items())[:3]:
...
async def test_executive_summary():
    """Test executive summary generation with PDF output."""
    ...
    # Generate summary
    logger.info("Generating executive summary with PDF...")
    updated_state = await exec_node.generate_executive_summary(state)
    
    # Get the generated summary and PDF
    ...
    if summary:
        ...
        output_path = f"/Users/erikivester/company-research-agent/pdfs/executive_summary_{timestamp}.md"
        with open(output_path, 'w') as f:
            f.write(summary)
        
        logger.info(f"✅ Saved executive summary to: {output_path}")
        
        # Save and open PDF if generated
        if pdf_buffer:
            pdf_path = f"/Users/erikivester/company-research-agent/pdfs/executive_summary_{timestamp}.pdf"
            with open(pdf_path, 'wb') as f:
                f.write(pdf_buffer.getvalue())
            
            pdf_size_kb = pdf_buffer.getbuffer().nbytes / 1024
            logger.info(f"✅ Saved executive summary PDF to: {pdf_path} ({pdf_size_kb:.2f} KB)")
            
            # Open the PDF
            subprocess.run(['open', pdf_path])
        else:
            logger.warning("⚠️ PDF was not generated")
