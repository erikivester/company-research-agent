    ...
    async def collect(self, state: ResearchState) -> ResearchState:
            ...
            if data and isinstance(
                ...
                if data_field == "company_brief_data":
                    ...
                    for url, doc in data.items():
                        ...
                        # Filter out social media and job board URLs
                        excluded_domains = ['linkedin.com', 'twitter.com', 'facebook.com', 'instagram.com', 'indeed.com', 'glassdoor.com']
                        is_excluded = any(domain in url.lower() for domain in excluded_domains)
                        if url and score > 0.0 and not is_excluded:
        ...
        if not best_url and all_scored_docs:
            ...
            if inferred_url and inferred_url.startswith("http"):
                ...
                logger.info(
                    f"Inferred company_url set to clean base URL: {clean_base_url} (from top score {all_scored_docs[0]['score']})"
                )
                msg.append(f"🔗 **Inlferred Company URL** set to: {clean_base_url}")
            else:
                logger.warning(
                    f"Top scored URL '{inferred_url}' was invalid, skipping URL inference."
                )
        elif best_url:
