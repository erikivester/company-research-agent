...
class Briefing:
    ...
    async def _update_airtable_status(self, record_id: str, status_text: str):
        ...
        try:
            ...
            logger.error(f"Briefing node failed to update Airtable status: {e}", exc_info=True)
    # --- END HELPER METHOD ---
...
""",
            ...
            'flw': f"""Create a focused briefing on {company}'s Food Loss & Waste (FLW) and Sustainability efforts, relevant to ReFED.
Key Requirements:
1. Structure using these exact headers ONLY IF relevant information is found in the documents. Use only bullet points under headers.
### FLW Initiatives & Reduction Efforts
* Specific actions mentioned for preventing food waste (e.g., forecasting improvements, shelf-life extension).
* Details on food rescue or donation programs mentioned (e.g., partners, volumes).
* Information on food waste recycling mentioned (e.g., composting, anaerobic digestion, depackaging).
### Sustainability Goals & Reporting
* Stated climate goals mentioned (especially methane reduction targets).
* Mention of sustainability reports (e.g., GRI, SASB) with year if specified.
* Environmental certifications or commitments mentioned (e.g., B Corp, UN SDGs).
### Packaging
* Details on packaging materials mentioned (e.g., recycled content, compostable).
* Mention of packaging optimization or reduction initiatives.
### Supply Chain Practices
* Mention of supply chain practices related to FLW (e.g., cold chain management, sourcing policies).
...
4. NEVER state "no information found" or "data not available".
5. Prioritize information directly stated in first-party sources (company website) if discernible from document context.
6. Provide only the briefing content in markdown format. No explanations or commentary.
"""
            # --- END ADDED PROMPT ---
        }
...
"""
    ...
    async def run(self, state: ResearchState) -> ResearchState:
        ...
        try:
             ...
             return state# backend/nodes/briefing.py
import asyncio
import logging
import os
from typing import Any, Dict, List, Union
