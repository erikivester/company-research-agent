...
class ContextPolisher:
    ...
    async def polish_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not content:
                logger.warning("No response content from Gemini model, returning original context")
                return context
            ...
            try:
                ...
                if not (content.startswith('{') and content.endswith('}')):
                    logger.warning("Response is not in JSON format, returning original context")
                    return context
                    
                polished = json.loads(content)
                if not isinstance(polished, dict):
                    logger.warning("Parsed response is not a dictionary, returning original context")
                    return context
