    ...
    def __init__(self):
        ...
        if not self.gemini_key and genai:
            ...
            try:
                ...
                logger.info("Initialized Gemini 2.0 Flash for executive summary generation")
            except Exception as e:
...
Now write the executive summary:"""
    ...
    def _summarize_sources(self, research_data: Dict[str, Any]) -> str:
        ...
        source_info = research_data.get('source_credibility', {}).get('reference_info', {})
        if not source_info:
...
def header_and_footer(canvas, doc):
    ...
    if os.path.exists(LOGO_PATH):
        canvas.drawImage(LOGO_PATH, doc.leftMargin, page_height - 0.8*inch, 
                         width=1.2*inch, preserveAspectRatio=True, mask='auto')
    
    # Header line
    canvas.setStrokeColor(REFED_GREEN)
    canvas.setLineWidth(1)
    canvas.line(doc.leftMargin, page_height - 1*inch, 
                page_width - doc.rightMargin, page_height - 1*inch)
    
    # Footer
    canvas.setFont(FONT_REGULAR, 8)
    canvas.setFillColor(REFED_DARK)
    footer_text = f"ReFED Research Brief • Generated {datetime.now().strftime('%B %d, %Y')}"
    canvas.drawString(doc.leftMargin, 0.5*inch, footer_text)
    
    if doc.page > 1:
        canvas.drawRightString(page_width - doc.rightMargin, 0.5*inch, f"Page {doc.page}")
    
    canvas.restoreState()


async def create_executive_summary_pdf(research_data: Dict[str, Any], output: io.BytesIO) -> None:
    """
    ...
    # Subtitle with URL
    if company_url:
        story.append(Paragraph(f'<a href="{company_url}" color="{REFED_GREEN}">{company_url}</a>', subtitle_style))
    else:
        story.append(Spacer(1, 20))
    
    # Convert markdown summary to PDF elements
    summary_elements = _convert_markdown_to_flowables(summary_text, heading_style, body_style)
...
def _convert_markdown_to_flowables(markdown_text: str, heading_style, body_style) -> list:
