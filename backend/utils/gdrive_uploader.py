...
def get_drive_service():
    ...
    if delegate_user:
        try:
            ...
            logger.warning(
                f"Failed to delegate credentials to {delegate_user}: {e}",
                exc_info=False,
            )
    ...
    try:
        ...
        if sa_email:
            logger.info(
                f"Google Drive credentials loaded for service account: {sa_email}"
            )
            ...
            try:
                ...
                if drive_count == 0:
                    logger.warning(
                        "⚠️ SA cannot see any Shared Drives! Add SA as member to the target Shared Drive."
                    )
                else:
                    logger.info(f"✓ SA can see {drive_count} Shared Drive(s)")
            except Exception as e:
                logger.warning(
                    f"⚠️ SA may not have permissions to list Shared Drives (this is ok if it's added as a member): {e}"
                )
    except Exception as e:
...
async def download_research_from_gdrive(folder_url: str) -> list[Dict[str, Any]]:
    ...
    try:
        def _list_and_download():
            ...
            logger.info("📡 Executing Google Drive API files().list() request...")
            results = (
                service.files()
                .list(
                    q=query,
        ...
        logger.info("🏃 Executing download operation in thread pool...")
        ...
        logger.error(
            f"💥 CRITICAL ERROR in download_research_from_gdrive: {e}", exc_info=True
        )
        return []
...
async def upload_context_to_gdrive(
    ...
    if not service:
        raise ConnectionError(
            "Failed to authenticate Google Drive service. Check credentials."
        )
    ...
    try:
        if content_type == "application/json":
            ...
            if hasattr(context, "read"):  # Check if it's a file-like object
                content = context.read()
            elif isinstance(context, io.BytesIO):
    ...
    media_buffer.seek(0)
    media = MediaIoBaseUpload(media_buffer, mimetype=content_type, resumable=True)
    ...
    try:
        ...
        def _execute_upload():
            ...
            if existing_file:
                ...
                logger.debug(
                    f"File '{file_name}' already exists. Updating existing file ID: {file_id}"
                )
        ...
        logger.error(
            f"Failed to upload file '{file_name}' to Google Drive: {e}", exc_info=True
        )
        raise
    finally:
        media_buffer.close()
...
async def upload_research_with_pdf(
    research_data: Dict[str, Any], folder_url: str, company_name: str
) -> Dict[str, str]:
    """
    ...
    # 1. Upload JSON data
    json_filename = (
        f"{company_name.lower().replace(' ', '_')}_research_{timestamp}.json"
    )
    try:
        await upload_context_to_gdrive(
            research_data, folder_url, json_filename, "application/json"
        )
    ...
    try:
        ...
        success, pdf_result = await pdf_service.generate_pdf_stream(
        ...
        if success:
            ...
            await upload_context_to_gdrive(
                pdf_buffer, folder_url, pdf_filename, "application/pdf"
            )
